...
engine = create_engine('postgresql://postgres:password@localhost/bi_casestudy')
meta = MetaData()
...
def purchase_builder(product_df, n=10000):
    ...
    for purchase_id in range(1, n):
        ...
        for i in range(purchase_order_size):
            ...
            unit_total = p.UnitPrice * unit_qty
...
def main():
    ...
    address = ['Auf dem Bruch 82a', 'Theodor-Gierath-Str. 1, Apt. 981', '03517 Legros Mountain, Suite 808',
               ...
               'Vedviks Gate 85, Leil. 287', 'Nygårds Vei 4, Oppgang A', 'Skolgatan 05, Lgh. 631', 'Södra Björkgränden 3']
    ...
    for i in range(1, 11):
        ...
        supplier_dict = {'SupplierID': i, 'CompanyName': Supplier.name, 'Address': Supplier.address,
    ...
    supplier_table.to_csv('BI_casestudy_testdata_suppliers.csv')
    ...
    region_table.to_csv('BI_casestudy_testdata_regions.csv')
    ...
    purchases.to_csv('BI_casestudy_testdata_purchases.csv')
    purchase_details.to_csv('BI_casestudy_testdata_purchasedetails.csv')
    ...
    sales_table.to_csv('BI_casestudy_testdata_sales.csv')
    sale_details_table.to_csv('BI_casestudy_testdata_saledetails.csv')
    time_table.to_csv('BI_casestudy_testdata_times.csv')
